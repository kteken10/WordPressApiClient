...
def update_button(page_id):
    ...
    # Extraire les boutons existants
    import re
    button_pattern = r'<a[^>]*href="([^"]+)"[^>]*style="([^"]*)">([^<]+)</a>'
    buttons = []
    
    for match in re.finditer(button_pattern, existing_content):
        link, style, label = match.groups()
        buttons.append({'link': link, 'style': style, 'label': label})
    ...
    new_style = input("Entrez le nouveau style CSS du bouton (laisser vide pour conserver le style existant) : ")

    # Si aucun nouveau style n'est entré, conserver le style existant
    if not new_style:
        new_style = buttons[button_index]['style']

    # Mettre à jour le bouton dans le contenu HTML
    button_to_update = buttons[button_index]
    updated_content = re.sub(
        rf'<a[^>]*href="{re.escape(button_to_update["link"])}"[^>]*style="{re.escape(button_to_update["style"])}"[^>]*>{re.escape(button_to_update["label"])}<\/a>',
        f'<a href="{new_link}" style="{new_style}">{new_label}</a>',
        existing_content
    )
    ...
    if update_response['status'] == "success":
        print(f"Le bouton a été mis à jour avec succès sur la page {page_id}.")
    else:
...
def delete_button(page_id):
    ...
    button_to_delete = buttons[button_index]
    updated_content = re.sub(
        rf'<a[^>]*href="{re.escape(button_to_delete["link"])}"[^>]*>{re.escape(button_to_delete["label"])}<\/a>',
        '',
        existing_content
    )

    # Mettre à jour la page sans le bouton supprimé
    update_response = pages_endpoint.update_page(page_id, content=updated_content)

    if update_response['status'] == "success":
        print(f"Le bouton a été supprimé avec succès de la page {page_id}.")
    else:
        print(f"Erreur lors de la suppression du bouton : {update_response['message']}")
create_table()